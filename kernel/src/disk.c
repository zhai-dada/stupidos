...
void write_handler(uint64_t nr, uint64_t parameter)
{
...
void other_handler(uint64_t nr, uint64_t parameter)
{
    struct block_buffer_node *node = ((struct request_queue *)parameter)->in_using;
    if (io_in8(PORT_DISK0_STATUS_CMD) & DISK_STATUS_ERROR)
    {
...
void read_handler(uint64_t nr, uint64_t parameter)
{
    struct block_buffer_node *node = ((struct request_queue *)parameter)->in_using;
    if (io_in8(PORT_DISK0_STATUS_CMD) & DISK_STATUS_ERROR)
    {
...
struct block_buffer_node *make_request(uint64_t cmd, uint64_t bloks, uint64_t count, uint8_t *buffer)
{
...
uint64_t disk_transfer(uint64_t cmd, uint64_t bloks, uint64_t count, uint8_t *buffer)
{
...
void disk_handler(uint64_t nr, uint64_t parameter, struct stackregs *reg)
{
    struct block_buffer_node *node = ((struct request_queue *)parameter)->in_using;
    if(node != NULL)
    {
...
void disk_init()
{
    ...
    register_irq(0x2e, &entry, &disk_handler, (uint64_t)&disk_request, &disk_controller, "disk0");
