...
{
    printk("do_invalid(10), ERROR_CODE:%#018lx, RSP:%#018lx, RIP:%#018lx\n", error_code, reg->rsp, reg->rip);
    if (error_code & 0x01)
    {
    ...
    {
        printk("Refers to a gate descriptor in the GDT or the current LDT\n");
    }
...
{
    printk("do_general_protection(13), ERR_CODE:%#018lx, RSP:%#018lx, RIP:%#018lx, RDX:%#018lx\n", error_code, reg->rsp, reg->rip, reg->rdx);
    if (error_code & 0x01)
    {
        printk("The exception occurred during delivery of an event external to the program, such as an interrupt or an earlier exception\n");
    }
...
void do_page_fault(stackregs_t *reg, u64 error_code)
{
    ...
    {
        printk("Page-level protection throws an exception,\t");
    }
    if (error_code & 0x02)
    {
