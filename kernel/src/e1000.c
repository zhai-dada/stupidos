...
static void recv_packet(e1000_t *e1000)
{
    while (1)
    {
        ...
        // color_printk(YELLOW, BLACK, "rx status 0x%X\n", rx->status);
        if (!(rx->status & RS_DD))
        {
            return;
        }
        ...
        if (rx->error)
        {
            color_printk(YELLOW, BLACK, "error 0x%X happened...\n", rx->error);
        }
        // TODO RECEIVE PACKET
...
void send_packet(eth_t *eth, uint16_t len)
{
    e1000_t *e1000 = &obj;
...
static void e1000_handler(uint32_t vector)
{
    ...
    uint32_t status = min32(e1000->membase + E1000_ICR);
    // color_printk(YELLOW, BLACK, "e1000 interrupt fired status %X\n", status);
    ...
    if ((status & IM_TXQE))
    {
        color_printk(YELLOW, BLACK, "e1000 TXQE...\n");
    }

    // 连接状态改变
    if (status & IM_LSC)
    {
        color_printk(YELLOW, BLACK, "e1000 link state changed...\n");
        // uint32_t e1000_status = min32(e1000->membase +  E1000_STATUS);
    ...
    if (status & IM_RXO)
    {
        color_printk(YELLOW, BLACK, "e1000 RXO...\n");
        // overrun
    }

    if (status & IM_RXDMT0)
    {
        color_printk(YELLOW, BLACK, "e1000 RXDMT0...\n");
    }
...
// 读取只读存储器
static uint16_t e1000_eeprom_read(e1000_t *e1000, uint8_t addr)
{
    uint32_t tmp;
    if (e1000->eeprom)
    {
...
static void e1000_read_mac(e1000_t *e1000)
{
    ...
    {
        uint8_t *mac = (uint8_t *)e1000->membase + 0x5400;
...
void e1000_init()
{
    ...
    if (!device)
    {
        color_printk(YELLOW, BLACK, "PCI e1000 ethernet card not exists...\n");
        return;
    }
