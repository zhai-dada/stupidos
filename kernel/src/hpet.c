...
void hpet_handler(uint64_t nr, uint64_t parameter, struct stackregs* reg)
{
    jiffies++;
    ...
    memset(&icr_entry, 0, sizeof(struct IRQ_CMD_REG));
...
void hpet_init()
{
    uint8_t* HPET_addr = (uint8_t*)P_TO_V(0xfed00000);
    ...
    *(uint64_t*)(HPET_addr + 0x100) = 0x004c;
    io_mfence();
    *(uint64_t*)(HPET_addr + 0x108) = 14318179 * 1; // 1 s
    io_mfence();
    get_cmos_time(&time);
