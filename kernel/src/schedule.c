...
void schedule()
{
    ...
    if(current->vruntime >= task->vruntime || current->state != TASK_RUNNING)
    {
        ...
        if(!task_schedule[smp_cpu_id()].exectask_jiffies)
        {
            switch(task->priority)
            {
                case 0:
                case 1:
                    task_schedule[smp_cpu_id()].exectask_jiffies = 4 / task_schedule[smp_cpu_id()].running_task_count;
                    break;
                case 2:
                default:
                    task_schedule[smp_cpu_id()].exectask_jiffies = 4 / task_schedule[smp_cpu_id()].running_task_count * 3;
                    break;
...
{
    int i = 0;
    memset(&task_schedule[0], 0, sizeof(struct schedule) * CPUNUM);
    for(i = 0; i < CPUNUM; i++)
    {
